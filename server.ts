...
async function startServer() {
  ...
  // API Route for generating course landing page with Gemini AI
  app.post("/api/generate-landing-page", async (req, res) => {
    console.log("Ricevuta richiesta di generazione landing page per:", req.body.title);
...
Restituisci esclusivamente l'oggetto JSON strutturato richiesto, senza markdown o commenti aggiuntivi.`;
      ...
      res.json({ success: true, landingPageData });
    } catch (err: any) {
      console.error("Errore generazione IA landing page:", err);
      res.status(500).json({ error: err.message || "Errore nella generazione con IA" });
    }
  });
