...
export const Navbar: React.FC<NavbarProps> = ({ user, onLogout, onNavigate, logoUrl, logoSize, logoOffsetX = 0, logoOffsetY = 0 }) => {
  ...
  return (
    <nav className="fixed w-full z-50 bg-white/90 backdrop-blur-md border-b border-gray-200 transition-all duration-300">
          ...
          <div className="flex items-center">
             ...
             <div className="hidden lg:flex items-center gap-6">
                <button 
                    ...
                    className="relative p-2 text-gray-600 hover:text-brand-600 transition-colors group"
                    title="Carrello"
                >
                    <ShoppingCart className="h-6 w-6 group-hover:scale-110 transition-transform" />
                    {totalItems > 0 && (
                        <span className="absolute -top-1 -right-1 inline-flex items-center justify-center h-5 w-5 text-xs font-bold leading-none text-white bg-brand-600 rounded-full border-2 border-white">
                        {totalItems}
                        </span>
                    )}
                </button>
                    ...
                    {user ? (
                        <div>
                            ...
                            >
                                ...
                                <div className="h-10 w-10 rounded-full bg-gray-100 flex items-center justify-center text-brand-600 border border-gray-200 shadow-sm group-hover:ring-2 group-hover:ring-brand-500/50 transition-all">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <button 
                                ...
                                className="bg-brand-600 text-white px-5 py-2.5 rounded-lg font-bold text-sm hover:bg-brand-700 transition-all shadow-lg shadow-brand-500/30 transform hover:-translate-y-0.5"
                            >
                                Inizia Ora
                            </button>
                        </div>
      ...
      {/* --- MOBILE MENU --- */}
      {isMobileMenuOpen && (
        <div className="lg:hidden bg-white border-t border-gray-200 absolute w-full left-0 shadow-2xl max-h-[calc(100vh-80px)] overflow-y-auto z-40 animate-in slide-in-from-top-2">
            <div className="px-4 py-6 space-y-4">
                ...
                {user ? (
                    ...
                    <div className="grid grid-cols-2 gap-4 px-4 pt-2">
                        ...
                        <button onClick={() => handleNavigate('/register')} className="w-full py-3 rounded-lg font-bold text-white bg-brand-600 shadow-lg">Registrati</button>
                    </div>
                )}
            </div>
        </div>
