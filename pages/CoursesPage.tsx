...
import { CheckCircle, ShoppingCart, Lock, Star } from 'lucide-react';
...
const CourseCard: React.FC<{ 
    ...
    // Calcolo prezzi (come in Home.tsx)
        ...
        <div className={`bg-slate-800 rounded-2xl overflow-hidden border border-slate-700 flex flex-col h-full group hover:border-brand-500 transition-all duration-300 shadow-xl ${isNotActive ? 'opacity-90' : ''}`}>
            ...
            <div className="relative h-56 overflow-hidden cursor-pointer" onClick={() => onCourseSelect(course.id)}>
                ...
                <div className="absolute top-4 right-4 flex flex-col gap-2 items-end">
                    ...
                    {!isNotActive && (
                        <div className="bg-brand-600 text-white px-3 py-1 rounded-full text-xs font-bold shadow-lg border border-brand-400">
                            -{discountPercent}%
                        </div>
                    )}
                    {isFull && (
            ...
            <div className="p-8 flex-1 flex flex-col">
                <div className="flex items-center justify-between mb-3">
                    <h3 className="text-xl font-bold text-white line-clamp-2 cursor-pointer group-hover:text-brand-400 transition-colors" onClick={() => onCourseSelect(course.id)}>{course.title}</h3>
...
export const CoursesPage: React.FC<CoursesPageProps> = ({ courses, onCourseSelect, user }) => {
  ...
  return (
    <div className="flex flex-col min-h-screen font-sans antialiased text-slate-100 bg-slate-950 selection:bg-brand-500/30 pt-32 pb-20 relative overflow-hidden">
